...
| `ai_jobs` | the Replicate job queue rows the client subscribes to |
...
- **`textHistory.ts` / `userColors.ts` / `photoEdits.ts` / `sync.ts`** — per-user persistence: recent texts, the saved palette, canvas archives, and synced settings.
