...
| Editor | `app.imagehorse.app` | `app/` | [`app/vercel.json`](../app/vercel.json) | **Vercel config written, not yet deployed** |
...
`app/vercel.json` is a port of the build command in `netlify.toml`, which stays
...
1. **Create the Vercel project** against this repo, and in its settings:
   - **Root Directory** = `app`. This is what makes Vercel read
     `app/vercel.json` rather than the repo-root one, which belongs to the
...
3. **Point `app.imagehorse.app` at the Vercel project** (CNAME to
...
`scripts/deploy-sentinel.sh` already defaults to `https://app.imagehorse.app`,
