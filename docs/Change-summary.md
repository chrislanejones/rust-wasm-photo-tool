...
## v8.80 Change Summary — 2026-09-20

**The font menu picks real fonts, object removal is a brush on the canvas, and an undo no longer comes back after a reload.**

| Area | Change |
| --- | --- |
| **Fonts** | `render_text`, `measure` and `wrap` take a `font_id`. The Text tool's font menu was cosmetic — the engine took no font parameter, so every choice drew the same face and three surfaces disagreed by up to **+26.3%**. Liberation Sans / Serif / Mono v2.1.5 (SIL OFL 1.1) ship as static assets in `app/public/fonts/` and are handed to the wasm at runtime through `register_font`. **No font bytes entered the wasm** — the one embedded face stays as the fallback and as what an empty `font_id` means. Subset with `pyftsubset` to the same **430 codepoints** the embedded face covers ([ADR-058](adr/058-fonts-arrive-at-runtime-and-dead-hinting-pays-for-them.md), #131). |
| **Engine size** | 845,156 → **814,202 B (−30,954)**, and **45,798 under** the 860,000 ceiling. Three new families were paid for by deleting hinting the rasterizer never ran: `ab_glyph` does not run the TrueType hinting interpreter, and the bulk of the waste was per-glyph instruction streams inside `glyf`, not the standalone tables. The swap gives back **63,109 B**. ⚠️ It could **not** have landed on master alone — 858,087 − 63,109 ≈ 794,978, which is **under the 800,000 floor** that detects a featureless build. It is only safe carried *with* the fonts feature. A permanent test, `fonts::the_embedded_faces_carry_no_hinting`, fails on any hinting table or non-zero glyph instruction stream, mutation-tested by restoring the original face — without it a stock TTF dropped in as a routine update silently spends 63 KB again (#131). |
| **Remove Object** | Selecting the tool puts the **main canvas** into mask mode: you brush over the object on the real image, in register, at whatever zoom you are on, and the app stays visible with the mask at 50% over it. The popup painted on a private copy of the frame **capped at 640 px wide** — about a third of actual size — with no zoom or pan while masking and a Clear that wiped everything. Now **Undo Stroke** and **Clear Mask**, and the mask stays over the object while the job runs. The mask bytes sent to the server are **identical** — the rasterizer moved verbatim (#178). |
| **Autosave** | ⚠️ **Silent data loss, and it was live.** Apply an edit, press Ctrl+Z, reload — and the change you had just discarded came back. Measured on `edit.imagehorse.app` before the fix and reproduced twice: after the undo the screen showed `77686a30` while the archive on disk still held `5b389c67`, unchanged even after 35 s, so Resume restored the pre-undo state. Undoing back to zero now writes (#161). |
| **Status bar** | **Undo NN%** — how deep undo can reach right now as a share of the History depth setting. 100% while the op log drives undo; otherwise `floor(history_max_bytes ÷ one whole-image copy)` clamped to 1..depth, counting a copy the way `Snapshot::bytes` does (every layer's W×H×4, the Canvas included, **plus the selection mask** while something is selected). It replaces a toast that fired once per photo. Always present, never red. The brand wordmark slides away after **5 min**, leaving the horse as the link (#186). |
| **Shape sloppiness** | A ramp instead of a switch — *"either human-drawn or computer-drawn, not much in between."* The cause was **three discontinuities the moment the slider left zero**, not a range that was merely too small, so raising the constants would not have fixed it. One is a bug in its own right: the firm branch drew a **circle** of radius `min(w,h)` while the sketchy branch drew the **bounding-box ellipse**, so any non-square drag changed shape *and* size at sloppiness 1 — and the fill, which uses the circle radius, had been sitting inside a wider outline the whole time. The end gap (which started at **9°**), the lead-in tail, and the tilt and squeeze constants all scale from zero now (#179). |
| **Mobile** | The mobile shell had no way into settings at all, so a phone was stuck on whatever theme it booted with. A gear in the header opens a bottom sheet with **Theme and Motion**, reusing `AppearancePane` whole rather than growing a second theme control. The entry point was measured at 390 px **before** it was chosen: a 44 px control leaves **84 px** of slack, no truncation, no page overflow (#173). |
| **Auth** | The app trusts the **production** Clerk instance (#175). |
| **Undo copy** | The degradation warning says what it means in words a reader has (#181). |
| **Marketing** | A blog, opening with the engine-in-a-worker post — its own scene, figures that move, tables and code, and it shares as one card (#167, #183, #185). An About page with real photos and Naji's story; the home-page tiles get their names and a press you can feel; a September hero with row edges and five moving figures; a hero slider that runs the edit backwards ([ADR-057](adr/057-threejs-is-a-marketing-only-dependency-behind-one-lazy-re-export.md), #159, #162, #163, #170, #172, #156). |
| **CI** | ⚠️ **A Convex deploy that deployed nothing reported success** — #165's job ran green three times with the secret unset and deployed nothing at all. The check now fails when it cannot verify, and says what to do about it: the deploy key needs `deployment:data:view` as well as `deployment:deploy` (#171, #180). The guardrails ratchet locks in three counts that came down (#182). |
| **Refactor** | The exif module split by what it knows about — 746 lines into 375 / 160 / 264 (#177). Step 0 of the sync work: an audit of what persistence does today, before any of it is designed (#164). Finished working documents moved out of the repository (#169). |
| **Sentinel** | The wasm ceiling is 860,000, and the script now says **why the floor is there, next to the floor** (#160, #184). |
| **QC** | `imagehorse-qc` had **not** completed for this cut. The engine and the canvas both changed (#131 fonts, #178 object-removal masking), so a full pass is owed and is the first thing to run against this build. Preflight gates were green: tsc 0, eslint 0 errors / 57 warnings, guardrails OK, cargo fmt + clippy `-D warnings` + tests clean, vitest **892 tests across 77 files**, production build clean, and the rebuilt wasm is **byte-identical to what production was already serving** (814,202 B, sha256 `369b9bef42ac45f4…`). |

## v8.81 Change Summary — 2026-09-21

**Twelve presets instead of six, H is the hand key, and the legal pages Google kept asking for are live.**

| Area | Change |
| --- | --- |
| **Presets** | Six more looks, so Enhance › Presets is **twelve** in four rows of three: **Mono** and **Noir** (the two black-and-whites — tones left where they were, versus hard contrast and heavy blacks), **Airy** (high-key), **Moody** (dark and saturated), **Recover** (pulls detail back out of a blown-out sky) and **Lift** (opens deep shadows on a backlit photo). They fill the gaps the first six left: nothing desaturated fully, nothing went high-key — the brightest was Enhance at +0.08 — Dark was neutral and gentle, and nothing moved the shadows/highlights pair far enough to matter. **Data only.** A preset is five numbers in a table `PresetsSettings.tsx` already held, so the engine, `src/presets.rs`, the op log and the undo contract are untouched and no Rust changed (#199). |
| **Why these six and not a Sepia** | ⚠️ **There is no hue or temperature knob in the stack**, and the shape of the table follows from that. `shadows` and `highlights` are luminance-masked *additive* shifts — the same delta goes to R, G and B (`simd::color`), so hue is preserved by construction. Every component moves luminance or saturation; none moves color. Warm and Cool have always been saturation, not temperature. A sepia or a split-tone cannot be a row in this table at all; it needs a filter the engine does not have. Now written above the table so the next person does not try. |
| **Preset test** | `e2e/preset-hover-preview.spec.ts` gains a case that hovers all twelve and compares canvas pixels to the untouched photo **and to every preset before it**. It exists for the one failure this table can have silently: `shadows`/`highlights` are **absolute 8-bit**, so a value that looks like its neighbours — `0.1` — is a tenth of one level out of 255 and contributes exactly nothing. tsc sees a valid number, the engine skips the component at its identity, the button still renders. Two presets carrying the same five numbers is the other silent one, and it fails here as a duplicate. |
| **Preset icons** | Three obvious picks were already spoken for **inside the same tool** and were dropped: `Aperture` is Levels, `SunDim` is Adjustments, and `Lightbulb` is the app's "here is an explanation" glyph everywhere (`info-tooltip.tsx`). The six use `Contrast`, `Clapperboard`, `Feather`, `CloudRain`, `Eclipse` and `Sunrise` — none used anywhere else in `app/src`. |
| **Hand key** | **H** pans: hold it and the pan ends on release, tap it and the pan latches until the next tap or Esc. Pan is one state fed by three sources (Space held, H held, H latched), so letting go of Space no longer ends a pan that H latched, and a window blur mid-hold ends the pan instead of leaving it stuck on. Esc releases the latch **without** `preventDefault`, so the same Esc still closes a dialog. Typing `h` in a text field does nothing, via the existing typing guard. ⚠️ **Space can never be the reliable pan key** — a keyboard-focused button takes Space, which is how Tab-then-Space activates it. H is not an activation key, so nothing focused can take it. Only `useKeyboardShortcuts.ts` changed; no AppShell edit (#188). |
| **Legal pages** | `/privacy-policy` and `/terms-of-service`. Google's OAuth consent screen will not pass the client without both on an authorized domain, and both were **404** — which is what was blocking Google sign-in. `Route` gains `footerOnly`: the pages stay in `ROUTES` so the sitemap (**8 → 10 URLs**) and the prerendered `<head>` (**7 → 9 pages**) pick them up, `PAGES` filters them out of the nav, and `LEGAL_PAGES` is the opposite filter for the footer. Nav, mobile sheet and ⌘K palette are unchanged at 7. ⚠️ **The paragraph a template would have got wrong is analytics**: GA4 runs on **both** hosts, **ungated**, and sets cookies — `app/src/lib/analytics.ts` says so in its own header, including that it is in tension with what the marketing site promises. Stated plainly rather than papered over. The terms stay short where the software earns it: no licence is granted over pictures that are never received (#196). |
| **Create AI Image** | The panel left `NewActions.tsx` — which was approaching the 900-line limit — for `CreateAIImagePanel.tsx`, and took the shared controls with it (`FieldLabel`, `SectionHeader`, `ToolButtonGroup`, `FIELD_SELECT`, `FIELD_TEXTAREA`) in place of hand-rolled form markup, so it matches the sidebar. It scrolls (`max-h-[70vh]`) on a short screen. **A model picker**: six Replicate models differing in speed, cost and reference-image support. Model choice lives in the parent because it is a preference that outlives the panel; the prompt and references are draft state that dies on the way back to the tile grid. References are kept across a model switch but only counted in the consent sentence when the chosen model can use them. Reference URLs are revoked in the `back` callback rather than an unmount effect, so a StrictMode remount does not reference a freed URL. Model IDs are Replicate slugs without version hashes — the server pins versions, so a client cannot reach an arbitrary model. **Generate is still disabled**: `convex/aiJobs.ts` has no text-to-image job type (#197). |
| **Engine size** | Unchanged at **814,202 B**. No Rust changed this release, so `pnpm run build:wasm` was a verify step only. |
| **QC** | ⚠️ `imagehorse-qc` has **not** completed for this cut, and the canvas and tools both changed (#188 pan, #199 presets), so a full pass is owed against this build. What was verified: preflight gates green — tsc 0, eslint 0 errors / 57 warnings, `guardrails.sh` OK, vitest **912 passed across 77 files**, `inert-class-audit` 0, production and marketing builds clean. And in a real browser on the production build in logged-out demo mode: the app boots, the fixture loads, the preset grid is 12 buttons in 4 rows with no panel scroll at 1440×900, all twelve preview distinct pixels on hover, leaving the grid restores the photo exactly, and one Ctrl+Z takes a preset back whole (`playwright preset-hover-preview`, **3 passed**, "Running 3 tests" confirmed). |

## v8.82 Change Summary — 2026-09-21

**Text keeps its typeface and its box through a reload, and the pricing page stops selling two things that don't exist.**

> ⚠️ **Correction (v8.83):** the typeface half of this was **not** fixed here. The `font_id` survived the reload; the face did not, because nothing on the resume path had registered the runtime fonts before the text was redrawn. Measured on a v8.82 production build: Mono glyphs 12.5–13 px apart when committed, i-stems 4.5–5 px after Resume. The box half was fixed. See v8.83.

| Area | Change |
| --- | --- |
| **Text, on reload** | ⚠️ **Live data loss since v8.80, found by the 09-19 QC (F1).** A committed text annotation lost its typeface and its dragged box on reload, and only on reload. Measured on production at `34dd0d9`: Liberation Mono ink **137 → 117 px** (came back proportional), Liberation Serif **94 → 103 px** (serifs gone). Two independent losses, either sufficient on its own. **(a)** `Op::TextEdit` apply in `src/ops.rs` REPLACED the annotation; `wrap_width`, `box_height`, `perspective` and `font_id` are `#[serde(skip)]` — they must be, or every payload on disk mis-decodes — so the op cannot carry them and replay honored the decode defaults as if the user had picked them. Apply now **merges** (`TextParams::carry_skipped_from`). The identical latent defect in `Op::ShapeEdit` for `sloppiness` and the quad is fixed with it (`ShapeParams::carry_skipped_from`). **(b)** `stripLiveAnnotations` in `app/src/lib/editPersistence.ts` was an **allowlist**: the engine has emitted `wrap_width` since v8.40, `box_height` since v8.41 and `perspective` since v8.42, and none had ever reached IndexedDB. It is a denylist now (everything the engine emits minus `tile_*`); `parseSnapshotAnnotations`, the second copy that actually fed `restoreLayerStack`, now **is** `stripLiveAnnotations`, and `restore_text_annotation` takes the three values instead of hard-coding `0, 0, identity` (ADR-060, #195). |
| **Op-log version** | **`OP_FORMAT_VERSION` stays 8, deliberately.** A bump is for a change in what the bytes are; the wire layout is untouched and a v8 writer and this reader agree frame for frame. `decode_op` accepts `2..=OP_FORMAT_VERSION`, so taking 9 would have made every shipped build reject new logs to fix a bug that needs no new bytes — and 9 is reserved for #187, which does. The honest cost: v8 bytes now replay differently than under v8.80, always toward what the user saw. Proven on captured production bytes by `tests/oplog_v8_text_settings_replay.rs` and `tests/oplog_v7_v8_fixture_resume.rs` (a v7 fixture's `wrap_width` comes back **338** instead of 0). `oplogManifests.formatVersion` is the literal `1` at both write sites and restore rejects anything else; the comment claiming it recorded `OP_FORMAT_VERSION` was wrong and was corrected, the field deliberately left alone — making it "honest" would have discarded every log on every disk. |
| **Drift guard** | `editPersistence.stripDrift.test.ts` asked the right question but built its input from `Required<PersistedAnnotation>` — the app's own allowlist — so a field the engine emits and the type never declared could not appear in it. It now derives the expected key set from the engine's captured JSON in **both** directions, and a Rust test compares that fixture against a live `get_text_annotations()`, so adding an engine field turns the Rust suite red first and names the TS file (#195). |
| **Pricing** | The page stops selling two things that do not exist. Pro's **"Cloud originals — 5 GB"** is now "5 GB of cloud storage", and originals are "on your device" on every tier: no tier uploads originals, and `StoragePane.tsx`'s Connect buttons are disabled. The **"Projects"** row is gone — the app makes zero `api.projects.*` calls and has no Projects screen — along with "3 projects" in the Free tier's Offer JSON-LD. Pro's AI allowance now reads **50 a day, 300 a month**, matching `TIER_MONTHLY_CAP.pro` in `convex/aiJobs.ts` (#208). |
| **Features page** | Caught up to what shipped: Adjustments, Levels and **Presets** (all twelve named, with the note that Warm and Cool are saturation, not temperature) replace one "Levels" entry; Liberation Sans/Serif/Mono replace "12 browser-safe options"; **38** sub-tools, not 34; H or Space to pan; undo 50 by default up to 1000; Magic Eraser shipped, Object Removal a LaMa brush on the canvas, Review on Alt+R. The generator stripped bold and code but not italics, so literal `*asterisks*` were rendering on the live page; fixed in `gen-trail-data.mjs`. Features 49 → **53** (#208). |
| **Architecture page** | The JSX map and the Mermaid file agree again: the edit archive goes to Convex for any signed-in user (not "paid: cloud backup"), both show PatchMatch and runtime fonts, and the engine grid is 12 boxes with all six filters named. The schema list says which 5 of the tables nothing reads. Parsed and rendered in Chromium with mermaid@11; a control copy with a bare `%%` failed, so the check can fail (#208). |
| **Site copy** | The blog post's **8 citation links all 404'd** — `repoFile` built `blob/main` and this repo has only `master`. Home's engine size is "roughly 350 KB gzipped" (measured live: 814,432 B raw, 356,517 B gzip). About's "Nothing you open is uploaded anywhere" is now "…uploaded to edit it", because signed-in archives and Pro AI do upload. Terms: `licence` → `license`, `sexualise` → `sexualize` (#208). |
| **TypeScript 7 readiness** | `marketing/tsconfig.json` drops `baseUrl` and makes its one path relative — the only tsconfig in the repo still carrying either, and the whole of the TS 7 config debt: under a real typescript@7.0.2, marketing goes from TS5102 + TS5090 to clean and `app/` was already at 0 errors. Identical mapping under 5.9.3. The bump itself (#205) stays parked: `typescript-eslint@8.70.0` declares `typescript <6.1.0` and exits 2 on 7.0 (#206). |
| **Engine size** | 814,202 → **814,432 B (+230)**, sha256 `d8afbefbdbacf868…`, 45,568 under the 860,000 ceiling. |
| **QC** | ⚠️ `imagehorse-qc` has **not** completed for this cut and is owed — the engine and text persistence both changed. A manual browser repro of the reload bug was attempted against a local production build and was **inconclusive**: the text never committed (Escape and switching tools both discarded it), so there was nothing to compare after reload. The fix rests on the Rust replay tests against captured production bytes. Preflight gates were green: tsc 0, eslint 0 errors / 57 warnings, guardrails OK, cargo fmt + clippy `--all-targets -D warnings` + **539** Rust tests clean, vitest **927 across 78 files**, `inert-class-audit` 0, production and marketing builds clean (sitemap 10 URLs). |

## v8.83 Change Summary — 2026-09-22

**Mono and Serif survive a reload for real this time, and the site paints in half the time.**

| Area | Change |
| --- | --- |
| **Text, on reload** | ⚠️ **v8.82 claimed this and it was false.** Found by the 09-21 `imagehorse-qc` pass: pick Liberation Mono, commit (click away, then Esc), reload, Resume — the text came back proportional. Glyph centers along `MMMM iiii MMMM` on a v8.82 production build: **12.5–13 px** apart committed, M's 17.5–18 px and i-stems **4.5–5 px** after Resume. #195 was right that `font_id` must survive, and it did. The face was lost one step later: runtime fonts (ADR-058) were registered ONLY by the Text and Batch panels, a reload opens on Enhance, and the engine rasterizes a `font_id` it has no bytes for in the embedded Sans — and a tile drawn that way does not redraw when the font arrives later. Isolated before it was explained: adding the registration and nothing else turned the reload test from red to pixel-identical (#210). |
| **Three paths, all missing it** | Resume from the op log (`restoreOplog` now awaits the faces before `oplog_restore`), resume from the archive (`restoreLayerStack` awaits them first), and **batch/ZIP export** — `compositeSavedEdit` builds a fresh engine and shares the archive helper, so a ZIP of a Mono photo shipped Sans. `ensureEngineFontsForRestore` is **bounded at 4 s**: a stalled fetch or wedged worker never hangs a resume, and the fallback is exactly the old behavior. The archive path waits only when a saved text names a runtime face (#210). |
| **Fonts, on a bad network** | Found reviewing #210: `loadFaces` and the per-engine registration both cached a failed fetch for the session, so one flaky moment removed Mono or Serif until the next reload — and #210 moved the first fetch to boot, when the network is busiest. Both caches now forget an incomplete result and the next caller retries; `resolveFacesWhenReady` already polls the Text panel for exactly that (#210). |
| **Tests** | Vitest pins the ORDER — the op-log replay and the archive restore are not reached until the faces resolve, the registrar gives up after its wait, a failed face is retried — each mutation-checked red. `e2e/text-face-survives-reload.spec.ts` is the browser proof: **red on v8.81 and on v8.82, green here**, with a second test proving Sans ≠ Mono in the pixels so the first cannot pass for the wrong reason. CI does not run Playwright, so the vitest tests are what hold it in CI. |
| **Site speed** | Home page, Lighthouse mobile, median: performance **64.5 → 97**, LCP **6.4 → 2.3 s**, FCP 3.0 → 1.6 s, weight 536 → 392 KiB; desktop 89 → **100**, layout shift 0.212 → **0**. Fonts are self-hosted and preloaded — the Google stylesheet was render-blocking and its late swap reflowed the headline (701 → 882 px at 1.4 s), which was the entire old layout shift; the CSP no longer names Google. Every page is its own chunk (entry 230 → 82 KB gzip; the Trail Log's 274 KB of release data loads only there). 800w/1200w hero encodes. The WebGPU cube letters start near the viewport and stop drawing once settled. The ⌘K palette and analytics load on idle (#209). |
| **Contact, footer** | `/contact`: email, bug reports on GitHub or Codeberg, a private route for security issues, account deletion, a link to chrislanejones.com — **no form**, because the site has nothing to receive one. In the footer, the sitemap (**10 → 11 URLs**) and ⌘K. The footer is three columns with a build-stamped copyright line (#209). |
| **Marketing consistency** | Every `@keyframes` and the reduced-motion block in `animations.css`; seven new tokens; Architecture's tier filter matches Pricing's and Home's; one shared "latest" pill; two dead selectors gone. Eleven visual calls are listed in PARKING_LOT, not applied (#209). |
| **Tab icon** | Both sites' tab icon is the horse on a rounded square of the site's near-black, `oklch(13% 0.018 35)`, in `favicon.svg` — not in `Image-Horse-Logo.svg`, which is also the mark drawn on the pages and must stay transparent. `apple-touch-icon` and the 192/512 manifest icons were still the old placeholder (dark square, orange tile); `gen-og-images.mjs` now rasterizes `favicon.svg` itself for them, via a new `--icons` flag (#211). |
| **Engine size** | Unchanged at **814,432 B**, sha256 `d8afbefbdbacf868…`. No Rust changed. |
| **QC** | `imagehorse-qc` ran against v8.82 on 09-21: the full e2e suite **26/26**; boot, file-picker and drag-drop import, H pan, export bytes, gallery reload and the preset focus ring passed; the text-reload row FAILED and is what this release fixes. §3 (button wiring) was being traced for this build at cut time. Preflight gates green: tsc 0, eslint 0 errors / 57 warnings, guardrails OK, cargo fmt + clippy `--all-targets -D warnings` + **539** Rust tests, vitest **933 across 79 files**, `inert-class-audit` 0, production and marketing builds clean (sitemap 11 URLs). |

## v8.84 Change Summary — 2026-09-22

**Your settings follow you between devices, and "Everything in your browser" now turns off every tool that uploads.**

| Area | Change |
| --- | --- |
| **Settings sync** | Signed in, three documents replicate: `prefs` (the Settings dialog), `ui` (master tab, panels, the palette's recent list) and `tools` (each tool's mode). Tabs on one machine sync over a BroadcastChannel; devices sync through a Convex `sync_docs` table (`sync:pull` / `sync:push` / `sync:clear`), with every push checked against the row's revision so an offline laptop can't overwrite a day of changes made elsewhere. **Never photos**: the archive stays out of sync (ADR-061). The online-features switch is deliberately not synced, because consent is per device. A signed-out change is not pushed on sign-in (the account wins), only the tab holding "Use here" pushes, and "Forget the synced copy" stays forgotten. `SyncErrorBoundary` + one toast per failure episode means a missing function degrades to tab-only sync, not a blank page. The functions were deployed to the live deployment before the merge (#207). |
| **Online-features switch** | The privacy policy said the AI tools aren't offered while the switch is off, and on v8.83 that was false: it hid only Create AI Image, while Enhance › AI (background and object removal) and Create › OCR still uploaded. The gate now keys off `requiresNetwork` in `toolGroups.ts`, the one list of what leaves the tab, and names no tool. It is enforced at four layers: `useAIJob.run` (the only upload path) refuses; `activateSubTool` refuses, which covers the rail, pasted links and the palette; the tiles and palette entries render disabled with a note; and switching off while one is open moves to the group's first tool. Settings › Security shows the same switch as the New dialog and lists what "on" turns on, built from the same flag. The privacy policy now names both places the switch lives (#198). |
| **Remove Object, Crop** | Leaving the AI tool mid-mask now does exactly what Cancel does: masking off, strokes and busy cleared. The overlay used to stay mounted and swallow every click. Apply Crop is disabled until a rectangle exists, the same gate the Enter key already had (#212). |
| **Canvas Size** | Remove and Resize sit side by side on one row, 80 + 70 px in the 226 px column, with a 38 px bar. Before, it was 246 px idle and 325 px with the `→ W×H` suffix, which wrapped into an 84 px two-row staircase. The visible labels are the verbs; the accessible names keep the object ("Resize canvas to W×H"). This resolves ADR-056's open question (#215). |
| **Shortcuts** | Alt + ← / Alt + → (the browser's Back and Forward) walk the tools you used, because every tool writes the route. It's now on the in-app list and in `docs/Keyboard-Shortcuts.md`, which also gains Enhance's Levels and Presets and the right sub-tool count, 38 not 34 (#214). |
| **One copy of each UI piece** | Nine shared components replace hand-written copies: select-field (6), number-field (11), confirm-dialog (6), pane-heading (13), status-note (9), segmented-tabs, swatch, ParkedScreen on dialog `size="sm"`, and HintTooltip with an IconButton `tooltip` prop. Repeated class strings and colors are named constants, and seven shadcn tokens now `var()` the brand token whose hex they repeated. Screenshot-diffed against master on 53 screens: seven deliberate changes, everything else pixel-identical. New Canvas's width and height are real `<label>`s now (#213, ADR-062). |
| **Dead code** | fallow on master: unused files **13 → 0**, unused exports **53 → 0**, duplicated lines **2,938 → 2,824**, main chunk **−26.7 kB** minified. `spike/` and the retired `toolConfig.ts` are deleted. The seven hand-run scripts fallow misread as dead are declared in `.fallowrc.jsonc`: `guardrails.sh` runs one of them (#213). |
| **Engine size** | Unchanged at **814,432 B**, sha256 `d8afbefbdbacf868…`. No Rust changed. |
| **QC** | Chris's user-test pass on 09-22 against the #213 preview (Settings panes, confirm dialogs, tooltips, New Canvas labels, dropdowns, Batch text, Perspective actions, palette tabs, Diagnostics, idle screen, compact mode, export) plus the Perspective, #212 and sync rows: "everything looks good". Screenshot diff, 53 surfaces. The e2e suite is 26 passed / 2 skipped. Its only failures were `pasted-export-name`, which fails **1 in 12 on master too** when runs alternate: a pre-existing flake, parked. Preflight gates green: tsc 0, eslint 0 errors / 57 warnings, guardrails OK, cargo fmt + clippy `--all-targets -D warnings` (with and without `tiles`) + **539** Rust tests, vitest **1026 across 90 files**, `inert-class-audit` 0, production and marketing builds clean (sitemap 11 URLs). Status board all green; Convex insights on the live deployment: no issues in 72 hours. |

## v8.85 Change Summary — 2026-09-22

**Sync gets its own page in Settings, on your phone too, with an off switch and a button that sends this device's settings.**

| Area | Change |
| --- | --- |
| **Why** | On v8.84 a Windows PC and an Android phone, both signed in, synced nothing. The live deployment showed `sync:pull` from both devices, **0 `sync:push`** and **0 `sync_docs` rows**. That is ADR-061's rule working as written: an account is never seeded from a device that hasn't changed anything, because letting the first device online win is how a fresh phone overwrites a laptop. The rule stays. What was missing was any way to see it, and any way to pick a side. |
| **Settings › Sync** | Its own pane (`#/settings/sync`, palette "Sync Settings"), moved out from the bottom of General. The same `SyncPane` renders in the phone's Settings sheet, which until now held only Appearance. Every control on it commits immediately, not on Apply. |
| **Sync on / Sync off** | Per device, kept in localStorage (`image-horse-sync-enabled`), never synced. Off behaves as signed out for the sync layer: the Convex query is skipped, nothing is sent, and a change made while off is owed to nobody. Tabs on the device still follow each other. On again is first contact: the device takes the account's copy, and anything it still owed from before is dropped. That's the reset, and it's the safe direction, because a device that was off for a week can't come back and overwrite the phone. |
| **Send this device's settings** | Shown only while the account holds nothing: no live row (Forget's markers count as empty) and no legacy `users.settings` blob. `sendThisDevice()` owes the account every document as this device holds it, once each store has loaded (5 s cap, so a store's defaults are never sent), and the usual push carries it. Only the tab holding the tab claim shows it. |
| **Privacy policy** | Names Settings › Sync, the per-device switch, and the Send button. |
| **Docs** | ADR-061 amended (it reverses the old "no switch, sign out instead" note). PARKING_LOT: `ToggleButtonGroup` emits no `aria-pressed`, so which button is on is visual-only to a screen reader. That was already true in every pane that uses it. |
| **Engine size** | Unchanged at **814,432 B**, sha256 `d8afbefbdbacf868…`. No Rust changed. No Convex change either, so nothing needed deploying before the merge. |
| **Gates** | tsc 0, eslint 0 errors / 57 warnings (baseline), guardrails OK, vitest **1039 across 91 files** (13 new), production build + `inert-class-audit` 0, marketing build clean, PR #216 CI 16/16 Actions green. Mutations: dropping the ledger forget, the fresh-session clear, the legacy check, or the Send body each turn a test red. |
| **QC** | Browser, local build: the desktop pane switches on → off → on and persists. At 390 px the phone sheet shows both buttons at 165×44 on one row with no horizontal scroll. The signed-in states ("Up to date", Send) need a real sign-in and are covered by the unit tests. The live check is the Send button on the PC, then the phone. |

## v8.86 Change Summary — 2026-09-22

**On the phone, Sync on and Sync off wait for Apply, and the worker post's message-size chart fits a phone screen.**

| Area | Change |
| --- | --- |
| **Phone › Settings › Sync** | On v8.85 the switch committed on tap, measured on live as localStorage flipping and the status line changing at once. Everything else in the phone sheet is a draft that Apply commits, so Apply stayed gray after a tap and Chris read the tap as broken, on a real phone. The sheet now drafts the switch next to theme and motion. A tap lights Apply and shows "Press Apply to turn sync off.", Apply commits whichever moved, and Cancel discards. The status line and the Send button still follow what the device is actually doing, never the draft. `SyncPane` takes an optional `draftEnabled` / `onDraftEnabledChange` pair. Without it the switch commits on tap, which the desktop pane keeps because that tab has no Apply (#217). |
| **Blog › FIG 3** | At 390 px the message-size table was 382 px wide inside a 287 px box. The label and value columns were both `nowrap` and the track has a 6rem floor, so the values were clipped: "~60 B" and "10 KiB" gone, "810× over" read "81". Below 40rem each row is two lines, the label and then bar + value. The value column is a fixed 5.5rem: sized `auto`, each row sized its own, and the tracks came out 285 → 242 px, which moved the dashed 10 KiB rule and bent the log scale row to row. Now every track is 197 px and the rule sits at 20.8% in every row. The table's roles are explicit, because changing a table's `display` drops its semantics in some screen readers. Desktop is unchanged (#218). |
| **Docs** | ADR-061's amendment corrected. It had said every Sync control commits at once. |
| **Engine size** | Unchanged at **814,432 B**, sha256 `d8afbefbdbacf868…`. No Rust or Convex change. |
| **Gates** | tsc 0, eslint 0 errors / 57 warnings (baseline), guardrails OK, vitest **1043 across 92 files** (4 new; with the sheet reverted to commit-on-tap, 3 go red), production build + `inert-class-audit` 0, marketing build clean. PR CI: #217 **16/16** and #218 **17/17** Actions green. |
| **QC** | Browser at 390 px, local build: tapping Sync off lights Apply and stores nothing; Apply stores "off", closes the sheet and toasts "Sync is off for this device." FIG 3 measured before (live) and after (local build): 0 values clipped, 5 equal tracks. The React #418 seen in local `vite preview` reproduces on master's build and not on live, so it's a preview artifact. |

## v8.87 Change Summary — 2026-09-22
...
| Area | Change |
| --- | --- |
| **Why** | `shares.listMine` had existed since the table was made ("for a future My links pane") and had zero callers. A copied link could never be seen again, and stopping one meant knowing its token. Chris, 09-22: "do something cool with shared files + views … someone might wanna revoke it after a certain amount of shares". He picked a view limit plus an end date, set afterward in Settings, where hitting a limit pauses the link rather than deleting it. |
| **Settings › Shared** | New tab (`#/settings/shared`, palette "Shared Links"). Totals across the top: links, views, most opened. One card per link with a thumbnail, a status (Live / Paused / Hit its view limit / Expired), "N views of M · last opened 3m ago · stops MM-DD-YYYY", and 30 day-bars built from the view log. Actions: "Stop after views" and "Stop on", a draft saved by Save limits; Copy link; Pause / Resume; Delete behind a confirm. Each commits as pressed, not on the Settings Apply. |
| **How a link stops** | Derived every time by one pure `availability(share, now)`, checked in order pause → date → cap. Nothing writes a "stopped" flag, so raising a cap or clearing a date un-stops a link with no extra write. "Stop on" takes effect at the start of that day. |
| **The public link** | `shares.get` returns a union. A live link gets the pre-feature fields plus `status: "live"`, so the v8.86 viewer kept working against the new backend in between. A stopped link gets `{status: "unavailable", reason}` and **no image URL**, withheld server-side. `recordView` counts live links only. The viewer says "turned off", "expired" or "reached its view limit" instead of "no longer available". |
| **View log** | New `share_views { shareId, at }`, one row per counted view, **timestamp only**: no IP, no browser, no account. The 30-day bars come from an index range, never a scan. Delete removes a link's rows with it. The privacy policy has a paragraph saying exactly this. |
| **End dates** | A Convex query is cached and not re-run as time passes, so `get` comparing the clock would have kept serving the image past the date until something touched the row. `setLimits` schedules internal `shares.expire` at `expiresAt`, and that write re-runs every cached and open read. A job for a date that was moved since does nothing. Disabling the schedule turns 2 tests red. |
| **Deploy order** | Functions were pushed to the live deployment (brave-ant-608) **before** the merge: **73 → 77** (`setLimits`, `pause`, `resume`, `expire`), none removed, the 3 existing links untouched. In a worktree, a bare `convex dev --once` went to a local anonymous backend and deployed nothing. Naming the deployment (`CONVEX_DEPLOYMENT=dev:brave-ant-608`) is what reached the real one. |
| **Tests** | 37 new: 32 in `shareLimits.test.ts` (the rule, the day buckets, every handler against an in-file fake with db / storage / scheduler) and 5 in `SharedPane.test.ts`. `MobileSettingsSheet.test.ts` now imports statically. Importing inside the test after `vi.resetModules` put the cold import inside the first test's 5 s budget, so it timed out under full-suite load, and then pressed Apply during the next test. |
| **Blog** | Second post, `/blog/offline-by-construction`: "The hotel Wi-Fi died. The editor kept running." It covers why editing survives a dead network (the engine is in a Web Worker, the truth is in IndexedDB) and what that buys a ward or an operating room. It has three WebGL figures. The blog index lists both posts, newest first, and the sitemap went **11 → 12 URLs** (2 posts). Measured before the merge on master + #219: both post pages prerender, and at 390 px no text block overflows, all three figures sit inside the page, and the two long code blocks scroll inside their own box (#219). |
| **Docs** | ADR-063 (draft). |
| **Engine size** | Unchanged at **814,432 B**, sha256 `d8afbefbdbacf868…`. No Rust changed. |
| **Gates** | tsc 0, eslint 0 errors / 57 warnings (baseline), guardrails OK, vitest **1080 across 94 files** in 5 consecutive full runs, production build + `inert-class-audit` 0, marketing build clean. PR CI: #219 **17/17** and #220 **17/17** Actions green. Features.md gains a share-links line, and the phone gear line now says Theme, Motion and Sync. |
| **QC** | Browser, local build: the Shared tab renders in the rail after Sync, and the pane shows its signed-out / connecting state. The signed-in card layout is covered by the tests but has not been seen in a browser, because a local build cannot sign in. The first live look is Chris's. |

## v8.88 Change Summary — 2026-09-22
...
| Area | Change |
| --- | --- |
| **Why** | Chris, 09-22: "how can you make it so you can test a roll out to a small amount of users — future thinking". Shipping went from Chris's machine to production for everyone with nothing in between. The in-between existed in pieces — `featureFlags.ts` has carried the opt-in experiments for releases — but the only way into one was typing its key into DevTools, so the ring was **one person wide**: everyone who could opt in already knew the key names. |
| **Three rings** | Ring 1 is Chris (Super User + the nine `ih_*` switches, 7 kill / 2 opt-in), already built. Ring 2 is this: invited people, per device, from Settings. Ring 3 — a percentage of signed-in accounts flipped from the server — is deliberately **not** built; it turns a feature on for someone who never asked, which is a different promise (ADR-064). |
| **Settings › Beta** | New tab (`#/settings/beta`, palette "Beta Features"). One row per experiment: On / Off, a reload note, and **Copy invite link**. Today: the Smart Brush (strokes that stop at an edge, ADR-014 — the engine side is tested, the *feel* has never been signed off) and blur on the graphics card (ADR-030). Switches commit as pressed; this pane has no Apply. |
| **No second registry** | The pane is a **view** of `featureFlags.ts` and names no key. A flag joins the ring by growing a `beta` block there; `lib/beta.ts` reads through `flag.isOn()` and writes through `setFlagOverride`, so the pane cannot disagree with the behavior and DevTools still works. ⚠️ The first cut held its own `{id, key}` list — the second-source-of-truth failure that registry's own header names — and was caught in review before merge. |
| **A stale row, found by building on it** | `ih_webgpu`'s description still said "no pixel in the app goes near the GPU yet". That stopped being true when whole-image blur grew its GPU path (`useTransforms.ts`, ADR-030). Fixed, with a note that a per-row registry cannot notice its own row going stale. |
| **The invite link** | `?beta=<id>` opts the device in, then `history.replaceState` removes **only** that parameter — a share link's `?v=` token and the `#/…` hash route both survive, pinned by a test. `?beta=none` clears every one. Applied once in `main.tsx`, before React renders and before the WebGPU gate reads its key. |
| **Privacy** | Nothing leaves the device: no account, no id, no request, and no privacy-policy change. That is also why a percentage of anonymous visitors was not built — it needs the per-browser id `identity.ts` deleted for being uploaded with no feature behind it. The cost is stated in the ADR: there is **no way to know who is in a beta**, so feedback has to be asked for rather than measured. |
| **Engine size** | Unchanged at **814,432 B**, sha256 `d8afbefbdbacf868…`. No Rust or Convex change. |
...
| Area | Change |
| --- | --- |
| **Mask state, said three times** (#246) | `useToolStore.maskEditing` now drives the Layer Settings tile, a status-bar chip (`Editing mask · black hides, white reveals`) and the brush ring, which wears the value the stroke will paint — ink `#000` or `#fff`, halo always the opposite so it survives a dark photo or a bright sky. None of the three computes its own answer. Mask editing previously *cleared* the cursor. |
| **Why the ring drops `difference`** | Differencing black against a photo is a no-op, so a black ring would vanish over dark pixels. The paired light/dark outline carries it over anything — the same trick the selection cursors use. |
| **§2 not built** (#246) | The plan's layer row — thumbnails, a ring, a `⋯` menu — has nothing to attach to: the layer picker is a native `<select>` whose `<option>` labels carry state as text. Building rows is a feature, not a consistency pass. Deferred whole, with the row menu behind it. |
| **Tool panel grammar** (#242) | One header, one row primitive, and an Advanced disclosure at the foot holding what most strokes never need. The closed header prints what is inside (`Stabilizer: Med`), so a setting that is ON is never hidden. |
| **Entropy refactor** (#247) | `drawEditState.ts`, `drawPreview.ts`, `canvasCursor.ts`, `shapeOverlayPath.ts`, `contractScan.ts` and `useCanvasCoords.ts` split out of `useDrawingTools.ts` and `CanvasArea.tsx`. Public APIs unchanged; `useDrawingTools` re-exports every moved name so no import moved. |
| **Engine** (#247) | Eleven wasm exports with no caller deleted, `resize_bilinear`'s input bounded, a copied bbox loop folded. **824,286 → 819,031 B, −5,255.** |
| **Three things the merge caught** | The extraction was correct when branched and stopped being correct while it waited. (1) `getCursorForSubTool` took `combineIntent` as a hand-written `0 \| 1 \| 2`, a copy of a union that had gained **Intersect (3)** — tsc caught it. (2) The extracted copy predated Night 4 and had no `maskEditing` branch; taking it as-is would have sent the mask brush back to the Move cursor with **no test and no type error**. (3) `has_layer_mask` was deleted as "no caller" — true when branched; `tests/selection_refine.rs` arrived afterwards with #233 and calls it twice. |
| **Which gate caught (3)** | Only `cargo clippy --all-targets` with **no features**, which is what the pre-push hook runs. `cargo test --features tiles,patchmatch` compiled it fine. Exactly the trap CLAUDE.md documents. |
| **Three max-lines caps raised** | AppShell 3649→3716, CanvasArea 2802→2823, the async contract test 1015→1027. **Merge arithmetic, not new slop:** this branch lowered the caps while master added Refine, Night 3 and Night 4 to the same files. Each merged number is still below what master permits (3718, 2909, and no per-file error cap at all). The reasoning is written into `eslint.config.mjs`. |
| **librs-lines stayed retired** | #247 reintroduced the `lib.rs` line ratchet at 4662. Master retired it on 09-25 with "Don't reintroduce it without asking" — master's side was kept. Flagged for Chris. |
| **Gates** | tsc 0, eslint **0 errors** / 54 warnings, vitest **111 files / 1242 tests**, `cargo clippy --all-targets` (no features) **0**, `cargo test` no-features 0, `cargo test --features tiles,patchmatch` **578 passed**, guardrails OK, app build 0. |
| **Verified in a browser** | Sample photo → Edit → Layers → Add mask → Show entire layer on the **merged** build: tile reads "Painting mask", the chip reads "Editing mask · black hides, white reveals", the ring is `#000` with a `#fff` halo and `mix-blend-mode: normal`. The hand-repaired cursor survives the refactor. |
