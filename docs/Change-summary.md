...
| 17  | Tests: **17 new.** `tests/shape_recolour.rs` (8) pins undo depth and restore, the annotation data behind reload, and the exported artifact; `useDrawingTools.test.ts` (9) pins the patch decision, including that selecting a shape without touching a control produces no patch and so costs no undo step. Rust 277 → 285, vitest 309 → 318 | Verified |
