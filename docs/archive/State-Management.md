...
- **`useToolStore`** persists the **pure sub-mode prefs** (`brushMode`, `stampSubMode`, `shapesMode`, `eraserMode`, `textMode`, `batchMode`) plus **`exportFormat`** and **`quality`** — all UI-level "remember what I picked" state with no engine coupling, so no rehydrate→WASM sync is needed. (Was four; `textMode`/`batchMode` joined in the toolbar restructure and `exportFormat`/`quality` in #14, where they had been AppShell `useState` and reset to JPEG/75 on every reload. The exact set is pinned by a drift test — see `stores/persistence.test.ts`, which fails on purpose when the allowlist widens.)
