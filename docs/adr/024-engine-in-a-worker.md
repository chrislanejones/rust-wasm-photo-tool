...
> nobody opened — and would serialize a 40-photo batch behind the open photo,
...
Stages 1 and 2 change **no behavior and introduce no worker**. They are worth
...
| **3.5** | **The async conversions — 168 → 7 as of v8.17. THE TARGET IS 5, NOT 0, AND THE LAST 2 ARE A PENOVERLAY REDESIGN, NOT A CONVERSION (see below).** ADDED 2026-08-07, **recounted 2026-08-08 — the old figures were an undercount, see below.** ⚠️ **This figure goes stale every batch; `scripts/engine-call-audit.mjs` is the authority and `engineAsyncMigration.contract.test.ts` pins it. If they disagree with this line, they are right.** `scripts/engine-call-audit.mjs` now counts 290 call sites: 101 fire-and-forget (a postMessage suffices), **162 value-consumed** (each needs a Promise *and* a call-site restructure), 27 hot-path. Until these are done the Stage 3 flag can never be turned on, so Stage 3's worker is scaffolding. The per-file batch list that used to sit here named six files; **four of them have since shipped** (`editPersistence` and `useEditPersistence` v7.84, `useLayers` v7.85, `useEngineCore` v7.90) and it was read as outstanding work for days after they were done. It is deliberately not replaced with another hand-written list — run the audit, which prints remaining-by-file. Same flag, still OFF. **a1 (measurable gate) v7.79 · a2 (text-metrics cache) v7.80 · a3 (one-call save capture) v7.84 · a4 (one-call pixels+dimensions capture) v7.88 — 121 → 103 · a5 (one-call UI-state capture) v7.90 — 103 → 94 · a7 (the hit-test capture) v7.94 — 93 → 92 · a7 cont. (the OpenRaster captures) v7.95 — 92 → 87 · **a8 scoping v7.97 — 87 → 81, RECLASSIFICATION not work** · **a8 batch 1 v7.98 — 81 → 76, `useHistory.ts` 5 → 0** (four truthy traps, same shape as `useLayers`; the ratchet was mutation-tested and DOES catch a dropped `await`, 5/5) · **a8 batch 2 v7.99 — 76 → 74** (`loadImageFromPixels`'s two reads) · **a8 scoping pass 2 v8.0 — 74 → 80, the number went UP** (19 discrete actions came back from a10; hot 32 → 13) · **a8 batch 3 v8.1 — 80 → 77** (`useTransforms`; also fixed two literal NUL bytes that made `textMetricsCache.ts` invisible to every grep, and found that **`textMetricsCache`'s 3 sites cannot be awaited at all** — they run in React's render pass and their documented resolution, `primeTextMetrics()`, was never written) · **a8 scoping pass 3 v8.2 — 77 → 73**: four sites in `lib/tilesFlush.ts` and `lib/oplogPersistence.ts` are called from `flushToCanvas` ACROSS A FILE BOUNDARY (`syncOplog`, `isLogTrustworthy`). The enclosing-name test only sees a function's OWN name, so a hot caller in another module is invisible to it — a new SHAPE of miss, not a new instance of an old one. · **a8 batch 4 v8.3 — 73 → 67**: the export layer (`useExport` 4→0, `lib/exportImage` 2→0). `generateThumbnailUrl` needed a RESTRUCTURE not an await — it checked its result inside a manual `new Promise` executor, where an async callee turns the guard into a truthy trap. · **a8 batch 5 v8.4 — 67 → 63**: clipboard copy + single export converted; `getHistogram` RECLASSIFIED — `HistogramView.sample()` calls it inside a `requestAnimationFrame` loop and it is a full composite pass (measured: 11,182,080 samples). **Third cross-file hot caller** after `syncOplog` and `isLogTrustworthy`. · **a8 batch 6 v8.5 — 63 → 59**: the four single-call files that needed no restructuring — `copy_region_composited` (Copy Selection), `capture_state` in BOTH save paths, and `export_png` (OCR). Three sat in already-async functions and one in a plain click handler. Picked deliberately as the batch with no rearranging in it; the three single-call files still left (`useExportDimensions`'s effect, and `useColorPicker`/`useCloneStamp`, whose `onMouseDown` handlers share the `Stamp["onMouseDown"]` type slot and so cannot be converted independently) each need it. · **a8 batch 7 v8.6 — 59 → 56**: the last three single-call files, all of which needed restructuring. `useExportDimensions`'s effect became an async IIFE with a CANCELLATION FLAG (React reads a returned Promise as the cleanup), load-bearing because its deps refire on every bbox-moving edit and the result is persisted to the Convex `shares` table — but the flag itself has NO TEST and is parked. `useColorPicker` and `useCloneStamp` had to convert TOGETHER: both fill the one `Stamp["onMouseDown"]` slot (`Stamp = ReturnType<typeof useCloneStamp>`), so either alone breaks the other's cast — a COUPLING THE AUDIT CANNOT SEE, since it counts sites per file and this constraint lives in a third. `useCloneStamp`'s was also a truthy trap (`if (!t.has_source())`), verified in the browser in both directions. · **a8 batch 8 v8.7 — 56 → 52**: `AISettings` 2 → 0 (two sync click handlers on `export_png`) and `usePastePlacementTool` 2 → 0, BOTH truthy traps on `has_paste_preview`. The `commit` one is the sharpest of the arc: `commit()` runs on EVERY tool switch on the understanding that it no-ops when nothing is pending, and the guard that makes it a no-op IS the converted call — so an un-awaited version puts `commit_paste_preview` on the engine every switch. Its three callers (keydown, pointerDOWN, effect) cannot await and are now `void commit()`. `update` stays hot. · **a8 batch 9 v8.8 — 52 → 39**: `useSelectionActions` 13 → 0, the LARGEST single batch of the arc, taking truthy-trap 6 → 1. `handleLassoMove`'s 2 sites stay hot and untouched — the reason this file was scoped last. Explicitly NOT an atomic capture: the lasso branch reads `lasso_active()`, MUTATES, then reads `lasso_committed_path()`, so a3/a7's one-capture fix does not apply. Two mechanics worth reusing: `await` goes on each BRANCH of a ternary (the audit reads the text immediately before the receiver, so `await (c ? a.f() : b.g())` counts as un-awaited), and `await tool?.m()` keeps an optional chain's short-circuit because `await undefined` stays falsy. All 13 driven in the browser; 13/13 mutants killed. · **a8 batch 10 v8.9 — 39 → 32**: `useDrawingTools` 7 → 0. The work was the CALL GRAPH, not the calls: `commitEdit` and `refreshShapes` have 4 and 5 internal callers and are exported. TWO callers had to `await` because the file's own comment makes the order a requirement — `onMouseDown` and `selectShape` both hit-test/read the annotation list that `commitEdit` writes, so fire-and-forget would race read against write and a click would miss the shape it just committed. The keydown and pointerDOWN listeners cannot await → `void commitEdit()`. The annotations-revision effect became an async IIFE with a cancellation flag (fires on every undo/redo). Browser witness: two pins number themselves 1 then 2, which only works if `dropPin` awaits the list read. 7/7 mutants killed. · **b1 INVESTIGATED, NOT BUILT (v8.10)** — `textMetricsCache`'s header claimed *every* call site tolerates a miss, and that claim was the whole basis for the `primeTextMetrics()` approach. It is FALSE: of six callers only the two RENDER sites fall back cleanly; `useTextTool`'s commit/re-edit pair drift, and `BatchSettings`' two throw BY DESIGN (their own comments say a miss becomes reachable under Stage 3.5). Worse, they would not throw: `Array.from(Promise)` is `[]`, which is TRUTHY, so `if (!m) throw` never fires and the corner-align arithmetic yields NaN for a whole batch — a truthy trap created by the conversion in a JS caller, invisible to the audit by construction. Corrected shape: split by whether the caller can AWAIT — 2 render sites keep the sync API + fallback and get primed off the render path; the 4 non-render callers await the engine directly. Header corrected in place; details in docs/PARKING_LOT.md. · **a8 batch 11 v8.12 — 32 → 26**: `useTextTool` 6 → 0 PLUS **b1's non-render half**, shipped together because two of b1's four callers live in this file. `textMetricsCache` gains `textInkOffsetBgAwaited` (same store + key as the sync form, so a commit warms the entry the next render reads — it doubles as the prime). The hit-test pair got ORDINARY awaits and explicitly not a capture (Capture 4 withdrawn). Verified by ROUND-TRIP: place text, re-open, re-commit unchanged → zero pixel drift. **8/8 mutants killed by TWO different gates** — the 6 engine sites by the ratchet, the 2 b1 callers by `tsc` (TS7053, indexing a Promise), because the ratchet counts engine calls and cannot see a dropped await on a JS wrapper. A wrapped call is covered only while its result is INDEXED. · **a8 batch 12 v8.13 — 26 → 21**: `BatchSettings` 5 → 0 plus **b1's last two callers** (b1 is now complete bar its render half). Breaks a six-batch streak — all five came out of UN-AWAITED (10 → 5), untouched since v8.5, because `applyToAll` was already async; what is left in that bucket is `openraster/export.ts` alone. The two `measureText` sites are what the b1 correction existed for: their `if (!m) throw` could never have fired under the sync form (`Array.from(Promise)` is `[]`, truthy), so awaiting REMOVES the miss rather than handling it. `get_image_data` needed the await INSIDE `new Uint8Array(...)`. Verified by running a real 4-photo batch: 5 `commit_text` calls, every dx/dy FINITE, and only ONE `measure_text` engine call for five stamps — the cache key is shared with the sync form. 6/6 mutants, again split between the ratchet (5) and tsc (2 b1 callers). · **b1 COMPLETE — a8 batch 13, v8.14 — 21 → 18**: `textMetricsCache` 3 → 0 by DELETION, not conversion. `cached()` is a pure lookup; the engine is reached only by the new `primeTextMetrics()`, called from a `CanvasArea` effect that bumps ONE re-render (the bump is load-bearing — priming without re-rendering leaves the fallback permanent and indistinguishable from working software). Sync `textInkOffsetBg` DELETED, zero callers since v8.12. **Built last on purpose and the ordering was the fix**: four callers could not tolerate a miss and moved to the awaited twins first (v8.12/v8.13). Browser-verified with a sharp discriminator — an unwired prime would mean ZERO engine calls; observed 0 → 1/1 on open → 2/2 on typing, and `transform-origin` [23.97, 10.65] against a fallback of [34.50, 20.45]. The cache's contract test was rewritten: three cases counted engine calls that can no longer happen, and the eviction case had gone VACUOUS (passing at size 0). · **a8 batch 14 v8.15 — 18 → 12**: `openraster/export.ts` 6 → 0. **The truthy-trap bucket reaches ZERO and the un-awaited bucket empties with it** (5 → 0, 1 → 0). The last trap was `flatten_text_annotations`, which reports whether a layer had annotations to bake — un-awaited it is truthy after the first layer, so every .ora export would have told the user their redo history was cleared. `set_active_layer` was NOT converted: fire-and-forget, never in the gate, despite a night plan listing it. **This does NOT make `exportOra` atomic** — Stage 3.5 WIDENS the gap (the `get_layer_png` loop gains N yield points), and b2's detect-don't-prevent counter is still unbuilt. Verified by exporting a real 11.5 MB .ora and parsing the archive: both layer PNGs present, `stack.xml` w/h matching the engine exactly. 6/6 mutants. · **a8 batch 15 v8.16 — 12 → 8**: four of `AppShell`'s six; un-awaited empties for good (1 → 0). **`handlePlace` carried a truthy trap the audit STRUCTURALLY CANNOT SEE** — `if (!moved)` sits a line BELOW the call, and classification reads the text at the call site, so the truthy bucket reported ZERO while this was live. "Truthy bucket empty" is a statement about the audit, not the codebase. **The two PEN sites were deliberately left**: `handlePenCommit` returns an id consumed by `PenOverlay.finish()` (6 call sites) and PenOverlay's UNMOUNT CLEANUP calls `onCommit` directly — a React cleanup cannot await, so async there means fire-and-forget during teardown. That is a state-machine change, not an await, and gets its own batch. 4/4 mutants; browser: place moved [418,615]→[191,324], share exportPng 5,570,242 B PNG. **a7 also withdrew one of its own two sites**: `useTextTool`'s hit-test-then-look-up has `commitText()` MUTATING between the two reads, so it is not an atomic capture at all and must not be converted — `docs/engine-worker-capture-sweep.md` is corrected, and the rule it yields is that **two sites with the same read sequence are not the same problem; what sits between the reads is part of the pattern.** a3 also found the category this ADR missed — see "ATOMIC CAPTURE" below, and triage every remaining file against it before converting · **a13 v8.17 — 8 → 7: `syncState`, the last conversion in Stage 3.5.** The most-traveled site in the migration (74 callers) and the one deferred longest, on a premise that was FALSE: the note below claimed all 74 "rely on it completing synchronously". None do — nothing consumes a return value, the only effect is `setState`, and React defers that anyway, so **no caller can observe a synchronous refresh today either**. Awaiting would have made 44 non-async handlers async and disturbed five hook files' shared type slots (the v8.6 shape) for a guarantee nobody had; FIFO, this ADR's own one-port invariant, already gives the ordering. What FIFO does not give is safety when the DOCUMENT IS REPLACED mid-flight: `reset()` nulls `toolRef.current` but **nothing calls `tool.free()`**, so the outgoing engine's capture resolves fine and lands the old photo's size and undo stack on top of the new one — silently, self-correcting on the next edit. Hence `readUiSnapshot(t, stillLive)`, extracted purely so the guard has a test; 3/3 compiling mutants killed (the hoist-above-the-await mutant is caught by only one of the seven tests, and a `tsc` failure was not counted as a kill). Also surfaced a **latent defect in `engineOwnership.contract.test.ts`**: its assignment scanner lacked `(?!=)`, so the first-ever comparison against `toolRef.current` was read as an unrouted assignment — its sibling check one test above had the guard, this one never did, in eight months. **The gate now stops at 7 and cannot reach 5 by conversion** — see the corrected done-condition below · **✅ THE PEN REDESIGN v8.19 — 7 → 5, THE FLOOR. STAGE 3.5 IS COMPLETE.** `handlePenCommit` / `handlePenHitTest`, built as `docs/pen-overlay-async-design.md` specified — a state-machine change, not awaits. Three pieces: the unmount cleanup stays fire-and-forget (it DISCARDS the id, so a13's reasoning applies unchanged — the blocker was always `finish()`, which consumes it); `finish()` goes async behind a re-entrancy guard, because an async `finish` can overlap itself and hold-Enter would otherwise commit the SAME anchors twice as two stacked paths; and `onCanvasDown` SPECULATES an anchor then corrects, because awaiting the hit test before deciding costs click-drag on the first anchor (`dragRef.current` must be set in the pointerdown's own tick). **A type slot the design missed and tsc could not have caught**: `CanvasArea`'s `onPenCommit` was declared `=> void`, and a function returning `Promise<number>` IS assignable to a `=> void` slot — the conversion would have typechecked while `typeof newId === "number"` silently went false and paths stopped staying selected. The v8.6 question "what shares this type" needs a corollary: **a `void` return is not proof that nothing is consumed.** Both guards were extracted to `features/canvas/penPath.ts` so they could be tested at all (no component-render harness in this repo — the a13 `readUiSnapshot` precedent), and both are invisible to the ratchet, tsc and eslint by construction. 4/4 mutants killed plus a deliberate EQUIVALENT mutant that correctly survived; 6 of 8 gesture cases driven in a foreground browser. ⚠️ **The browser did NOT prove either guard and was not reported as having done so** — the port is an identity function today, so the re-entrancy window never opens (12 Enter presses gave one path, but would have anyway). The mutation run proves the guards; the browser proves the positive path, of which the closed-loop case is decisive: "stayed selected" is only possible if the awaited id came back a real number | nothing user-visible | flag stays off |
...
**✅ REACHED v8.19 — `remaining − 5` is 0. STAGE 3.5 IS COMPLETE.** The last two
were `AppShell`'s pen pair, and v8.17 was right that they were not `await` jobs:
`handlePenCommit`'s return value is consumed synchronously by
`PenOverlay.finish()` **and** by the overlay's unmount cleanup, where a React
cleanup function cannot await; `handlePenHitTest` runs inside mousedown to fork
between re-opening a path and starting one, a decision the overlay's state
machine had no pending state for. They shipped as a **PenOverlay redesign** on
its own session, not as a conversion batch.

Of the two halves, only one was a real blocker. The unmount cleanup **discards**
the id, so fire-and-forget is correct there for exactly a13's reason — nothing
consumes the result, and the request is posted synchronously onto a FIFO port.
"A cleanup cannot await" read like the obstacle for six releases and was not one.
The obstacle was `finish()`, which does consume the id, and which needed a
re-entrancy guard once it went async: hold `Enter` and an unguarded async
`finish` commits the same anchors twice, as two identical paths stacked on top of
each other, one undo step each.

⚠️ **THE UNMOUNT PATH IS NOT CLEARED BY THIS STAGE, AND THE GREEN CHECK SAYS
OTHERWISE IF READ CARELESSLY.** "Switch tool mid-draw → committed, undo 16 → 17"
is a NO-REGRESSION result. With the flag off, `attachLivePort` is an identity
function and the `await` in `handlePenCommit` resolves in a **microtask**, before
React finishes teardown — so the dangerous window does not exist yet and a pass
cannot distinguish "safe" from "never exercised". Behind the worker it is a round
trip, and the cleanup's commit becomes a message posted by a component that is
already gone. The safety argument (posted synchronously, FIFO port, nothing
consumes the reply) is written down but is **read from `src/ops.rs` and
`port.ts`, never measured**. That test belongs in **a12 with the flag ON**, with
the cross-implementation matrix.

**THE THREE TRAP SHAPES THE AUDIT CANNOT SEE**, consolidated here because they
are scattered through the ledger above and each cost a batch to find:

| # | Shape | Found | Why the audit is blind to it |
|---|---|---|---|
| 1 | **Conversion-created, in a JS caller** — `generateThumbnailUrl` checked its result inside a `new Promise` executor | v8.3 | the trap does not exist until the callee becomes async; nothing to classify beforehand |
| 2 | **Wrapped call** — `Array.from(Promise)` is `[]`, which is **truthy**, so `if (!m) throw` never fires | v8.10 / v8.13 | the engine call is awaited *inside* the wrapper, so a dropped `await` at the caller is structurally invisible to a ratchet that counts engine calls |
| 3 | **Guard on the NEXT line** — `const moved = tool.align_annotation(...)` then `if (!moved) return;` | v8.16 | `inCondition()` inspects the text at the call site; a guard one line below is out of frame |

A fourth, from this stage: **a `void` return slot silently swallows a Promise.**
`Promise<number>` is assignable to `=> void`, so `tsc` accepts it. Not an audit
...
counts down invites the wrong fix, and a gate at its floor invites the assumption
that the floor is a defect.

**`syncState` — DONE v8.17 (a13), and it did NOT need its 74 callers.** The note
here previously said awaiting `capture_ui_state()` "makes `syncState` async across
74 call sites, all of which currently rely on it completing synchronously." The
second half was wrong, and it is the reason this sat deferred since v7.99. No
caller relies on completion: nothing consumes a return value, the only effect is
`setState`, and React defers that anyway — so **no caller can observe the mirror
being refreshed synchronously today either.** Awaiting would have turned 44
non-async handlers async and disturbed the shared type slots of five hook files
(the v8.6 shape) to buy a guarantee nobody had. `syncState` is async;
all 74 sites call it unchanged, and **FIFO** — already this ADR's own invariant,
one port per document — is what makes the ordering safe.

What FIFO does not cover is the document being replaced mid-flight, so the
capture moved into an exported `readUiSnapshot(t, stillLive)` with a liveness
guard. `reset()` nulls `toolRef.current` but **nothing calls `tool.free()`**, so
the outgoing engine's capture resolves normally and would land the old photo's
size and undo stack on top of the new one — silently, self-correcting on the next
edit. The guard compares engine identity, checked **after** the await; the
extraction exists so it has a test, and the mutant that hoists it above the await
is the only one three of its seven tests can catch.
...
**The generalisable bit: before optimizing a hot read, check whether anything
...
⚠️ **That same behavior is a hazard in the other direction, and it is the
...
- **For:** buys most of the benefit for a fraction of the risk. The audit shows
  the pain is concentrated in batch/serialization files —
