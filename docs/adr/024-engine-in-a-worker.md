...
**⚠️ CORRECTED v8.17 — `remaining − 5` CANNOT reach 0 by conversion.** The gate
stands at 7, and the 2 non-exempt sites left are `AppShell`'s pen pair, which are
not `await` jobs at all: `handlePenCommit`'s return value is consumed
synchronously by `PenOverlay.finish()` **and** by the overlay's unmount cleanup,
where a React cleanup function cannot await; `handlePenHitTest` runs inside
mousedown to fork between re-opening a path and starting one, a decision the
overlay's state machine has no pending state for. Stage 3.5's last step is a
**PenOverlay redesign**, scheduled on its own — not another conversion batch.
Both sites are now pinned by name in a contract test that fails if they are
converted, for the same reason `DISSOLVES_AT_STAGE_4` exists: a gate that only
counts down invites the wrong fix.
