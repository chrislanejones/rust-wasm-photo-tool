...
| [016](016-canvas-is-document-metadata.md) | The Canvas is document metadata, not a logged layer | Draft — unblocks op-log undo/persistence on the DEFAULT document (amends 006/012/013); engine + UI BUILT, flags still OFF; needs the real-gallery check on a DEFAULT document. **Amended 2026-07-14: the 07-14 flag-flip A/B proved the pre-mortem's data loss (flags ON: 220×170 edit-free reload → 200×150, Canvas destroyed) — an empty log's lazily-captured base predated `set_artboard_border`, was persisted as restorable, and its "successful" restore skipped the working-copy fallback. Fixed on fix/oplog-canvas-metadata: engine rebases stale empty bases at recomposite; zero-op logs are never persisted and never restore; e2e A/B pin added** | 2026-07-13 |
