...
## OPEN — `tests/oplog_v3_resume.rs` has no `required-features` block (2026-08-17)

⚠️ **This blocks `cargo clippy --all-targets`, which is what the pre-push hook
runs — and it is on MASTER's uncommitted v8.41 work, not the perspective
branch.** Control-verified: master's own unmodified copy of the file fails
identically with my edit reverted.

Every other tiles-dependent integration test declares a `[[test]]` block in
`Cargo.toml` with `required-features = ["tiles"]`. `oplog_v3_resume` does not,
so Cargo builds it for the DEFAULT feature set, where the `ops` module does not
exist at all.

| Command | Result |
|---|---|
| `cargo test --features tiles,patchmatch` | **passes** |
| `cargo clippy --all-targets` (no features, = the hook) | **fails to compile** |

Exactly the split CLAUDE.md's Commands section warns about. The fix is the
three-line block, applied on `feat/perspective-tool`:

```toml
[[test]]
name = "oplog_v3_resume"
required-features = ["tiles"]
```

**If the v8.41 commit lands the same block, expect a trivial Cargo.toml
conflict — take either copy, they are identical.**
