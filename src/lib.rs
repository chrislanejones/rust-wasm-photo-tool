    ...
    fn oplog_sync_annotations(&mut self) {
        ...
        let mut pending: Vec<crate::ops::Op> = Vec::new();
        {
            ...
            for s in &layer.shape_annotations {
                ...
                // ⚠️ `ShapeParams::corner_radius` is `#[serde(skip)]` (it has
                // to be — see the field's comment), so `ShapeAdd`/`ShapeEdit`
                // physically CANNOT carry it. It rides as its own
                // `ShapeCornerRadius` op — the TextWrap recipe above, applied
                // to shapes — or replay rebuilds the rectangle square, the
                // composite hash diverges, and the log marks itself broken.
                match log_doc.shapes.iter().find(|p| p.id == s.id) {
                    None => {
                        let radius = params.corner_radius;
                        pending.push(crate::ops::Op::ShapeAdd(params));
                        if radius != 0 {
                            pending.push(crate::ops::Op::ShapeCornerRadius { id: s.id, radius });
                        }
                    }
                    Some(p) => {
                        if p.corner_radius != params.corner_radius {
                            pending.push(crate::ops::Op::ShapeCornerRadius {
                                id: s.id,
                                radius: params.corner_radius,
                            });
                        }
                        // Compare everything EXCEPT the skipped field, which
                        // the branch above already accounted for — otherwise a
                        // radius-only change would also emit a redundant
                        // ShapeEdit.
                        let mut without_radius = p.clone();
                        without_radius.corner_radius = params.corner_radius;
                        if without_radius != params {
                            pending.push(crate::ops::Op::ShapeEdit(params));
                        }
                    }
                }
            }
