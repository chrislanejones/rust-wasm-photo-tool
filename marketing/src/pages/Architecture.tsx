...
function Node({
  ...
  return (
    <div className={`rounded-lg border ${NODE_COLORS[color]} ${padding} ${
      highlighted ? "ring-2 ring-violet-500/50" : ""
    }`}>
      <div className="flex items-center justify-between">
        <div className={`font-medium ${titleSize}`}>{title}</div>
        {badge && (
          <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${
            BADGE_COLORS[badge] || "bg-zinc-700 text-zinc-400"
          }`}>{badge}</span>
        )}
      </div>
...
function Label({ color, children }: { color: keyof typeof LABEL_COLORS; children: React.ReactNode }) {
  const [t, dot] = LABEL_COLORS[color];
  return (
    <div className={`text-xs font-medium uppercase tracking-wider flex items-center gap-2 mt-4 ${t}`}>
...
type Field = { name: string; type: string; key?: "pk" | "fk" | "unique"; indexed?: boolean };

function TableCard({
  name, fields, indexes, badge, note,
}: {
  name: string; fields: Field[]; indexes?: string[]; badge?: string; note?: string;
}) {
  return (
    <div className="rounded-lg border border-pink-500/30 bg-pink-500/5 overflow-hidden">
      ...
      <div className="p-2 space-y-0.5 text-[10px] font-mono">
        {fields.map((f) => {
          const ic =
            f.key === "pk" ? "text-amber-400"
            : f.key === "fk" ? "text-blue-400"
            : f.key === "unique" ? "text-emerald-400"
            : "text-zinc-500";
          const icon = f.key === "pk" ? "🔑" : f.key === "fk" ? "🔗" : f.key === "unique" ? "◆" : "·";
          return (
            <div key={f.name} className="flex items-start gap-1">
              <span className={`shrink-0 ${ic}`}>{icon}</span>
              <span className="text-pink-200">{f.name}</span>
              <span className="text-zinc-600">:</span>
              <span className="text-zinc-400 truncate">{f.type}</span>
              {f.indexed && <span className="text-amber-500/60 text-[8px]">idx</span>}
            </div>
          );
        })}
      </div>
      {indexes && indexes.length > 0 && (
        <div className="px-2 pb-2 pt-1 border-t border-pink-500/10">
          ...
          <div className="flex flex-wrap gap-1">
            {indexes.map((idx) => (
              <span key={idx} className="text-[9px] px-1 py-0.5 rounded bg-amber-500/10 text-amber-400/80 font-mono">
                {idx}
              </span>
            ))}
          </div>
...
const TABLES: Array<{
  name: string; badge?: string; fields: Field[]; indexes?: string[]; note?: string;
}> = [
  {
    name: "users",
    fields: [
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 p-8 overflow-x-auto">
      <header className="max-w-7xl mx-auto mb-8">
        <div className="flex items-center gap-3 mb-2">
          <img src="/Image-Horse-Logo.svg" alt="" className="w-9 h-9 rounded-lg" />
          <h1 className="text-2xl font-semibold tracking-tight">Image Annotation App</h1>
        </div>
        <p className="text-zinc-500 text-sm">
          Backend Architecture · v2.0 — <span className="text-orange-400 font-medium">+ WASM / Rust</span>
        </p>
      </header>
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="mt-10 pt-6 border-t border-zinc-800">
          ...
          <div className="mt-6 rounded-lg border border-pink-500/20 bg-pink-500/5 p-4">
            ...
            <div className="flex items-center justify-center gap-2 text-xs text-zinc-400 flex-wrap">
              {["users", "projects", "images", "layers", "annotations"].map((t, i, arr) => (
                <span key={t} className="contents">
                  <span className="px-2 py-1 rounded bg-pink-500/20 text-pink-300">{t}</span>
                  {i < arr.length - 1 && <span className="text-zinc-600">1 ─── ∞</span>}
                </span>
              ))}
            </div>
        ...
        <div className="mt-10 pt-6 border-t border-zinc-800">
          <div className="flex flex-wrap gap-6 text-xs text-zinc-500">
            <Legend color="#3b82f6" label="Client Flow" />
            <Legend color="#f97316" label="WASM / Rust" />
            <Legend color="#10b981" label="API Flow" />
            <Legend color="#8b5cf6" label="AI Processing" />
            <Legend color="#f59e0b" label="Storage" />
            <Legend color="#ec4899" label="Database" />
            <Legend color="#71717a" label="Webhooks" dashed />
          </div>
        </div>
        ...
        <div className="mt-12 text-center">
          <Link
            to="/#pricing"
            className="inline-block px-5 py-3 rounded-lg bg-gradient-to-br from-orange-500 to-red-500 text-white font-medium hover:opacity-90 transition"
          >
            See pricing →
          </Link>
        </div>
